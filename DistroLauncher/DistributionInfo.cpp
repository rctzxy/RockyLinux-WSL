...
bool DistributionInfo::CreateUser(std::wstring_view userName)
{
    ...
    HRESULT hr = g_pWslApi->WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
    ...
    commandLine += userName;
    hr = g_pWslApi->WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
    if ((FAILED(hr)) || (exitCode != 0)) {
        return false;
    }
    ...
    if ((FAILED(hr)) || (exitCode != 0)) {
        ...
        commandLine += userName;
        g_pWslApi->WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
        return false;
    }
...
ULONG DistributionInfo::QueryUid(std::wstring_view userName)
{
    ...
    if (CreatePipe(&readPipe, &writePipe, &sa, 0)) {
        ...
        int returnValue = 0;
        HANDLE child;
        HRESULT hr = g_pWslApi->WslLaunch(command.c_str(), true, GetStdHandle(STD_INPUT_HANDLE), writePipe, GetStdHandle(STD_ERROR_HANDLE), &child);
        if (SUCCEEDED(hr)) {
