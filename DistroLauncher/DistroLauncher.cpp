...
WslApiLoader* g_pWslApi = nullptr;
...
HRESULT InstallDistribution(bool createUser)
{
    ...
    HRESULT hr = g_pWslApi->WslRegisterDistribution();
    if (FAILED(hr)) {
        return hr;
    }
    ...
    DWORD exitCode;
    hr = g_pWslApi->WslLaunchInteractive(L"rm /etc/resolv.conf", true, &exitCode);
    if (FAILED(hr)) {
        return hr;
    }

    Systemd::Configure(true);
...
HRESULT SetDefaultUser(std::wstring_view userName)
{
    ...
    HRESULT hr = g_pWslApi->WslConfigureDistribution(uid, WSL_DISTRIBUTION_FLAGS_DEFAULT);
    if (FAILED(hr)) {
        return hr;
    }
...
{
    // Set WSL name.
    wchar_t* fileName = new wchar_t[wcslen(argv[0]) + 1];
    wcscpy_s(fileName, wcslen(argv[0]) + 1, argv[0]);
    ...
    g_pWslApi = new WslApiLoader(std::wstring(fileName));
    ...
    DWORD exitCode = 1;
    if (!g_pWslApi->WslIsOptionalComponentInstalled()) {
    ...
    HRESULT hr = S_OK;
    if (!g_pWslApi->WslIsDistributionRegistered()) {
    ...
    if ((SUCCEEDED(hr)) && (!installOnly)) {
        if (arguments.empty()) {
            hr = g_pWslApi->WslLaunchInteractive(L"", false, &exitCode);
            ...
            hr = g_pWslApi->WslLaunchInteractive(command.c_str(), true, &exitCode);
